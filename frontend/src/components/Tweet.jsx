...
function Tweet({ tweet }) {
  ...
  const { userDetails, description, createdAt, _id, like } = tweet || {};
  const userDetail = userDetails ? userDetails[0] : {};
  const userImage = userDetail?.profileImage || "https://img.freepik.com/premium-photo/3d-cartoon-software-developer-generative-ai_644690-101590.jpg";

  return (
    <div className="border-b border-gray-200">
      <div className="flex p-4">
        <Avatar src={userImage} size="45" round={true} />
        <div className="ml-2 w-full">
          <div className="flex items-center">
            <h1 className="font-bold">{userDetail?.name}</h1>
            <p className="text-gray-500 text-sm ml-1">
              {`@${userDetail?.username} . ${timeSince(createdAt)}`}
            </p>
          </div>
          <p>{description}</p>
          <div className="flex justify-between my-2">
            ...
            <div className="flex items-center">
              <div className="p-2 hover:bg-green-200 rounded-full cursor-pointer" aria-label="Comment">
                <TfiComments size={"22px"} />
              </div>
              <p>0</p>
            </div>
            <div className="flex items-center">
              <div className="p-2 hover:bg-sky-200 rounded-full cursor-pointer" aria-label="Bookmark">
                <GoBookmark size="23px" />
              </div>
              <p>0</p>
            </div>
            {user?._id === tweet?.userId && (
              <div onClick={() => deleteTweetHandler(_id)} className="flex items-center">
                <div className="p-2 hover:bg-red-300 rounded-full cursor-pointer" aria-label="Delete Tweet">
                  <MdDeleteOutline size="24px" />
                </div>
              </div>
            )}
