...
const useGetMyTweet = (id) => {
  ...
  const followingTweetHandler = async () => {
    try {
      const res = await axios.get(
        `${TWEET_API_END_POINT}/followingtweets/${id}`,
        { withCredentials: true } // Set withCredentials for this specific request
      );
      console.log(res);
      dispatch(getAllTweet(res.data.tweets));
    } catch (error) {
  ...
  }, [isActive, refresh, dispatch, id]); // Added 'dispatch' and 'id' to dependency array for completeness
};
