...
export async function Register(req, res) {
    try {
        ...
        const hashedPassword = await bcryptjs.hash(password, 10); // Reduced rounds for better performance
        await User.create({
            name,
            username,
            email,
...
export async function Login(req, res) {
    try {
        ...
        return res
            .status(200)
            .cookie("token", token, { maxAge: 3600000, httpOnly: true }) // Changed expiresIn to maxAge (milliseconds)
            .json({
                message: `Welcome back ${user.name}`,
                user,
                success: true
...
export async function follow(req, res) {
    try {
        const loggedInUserId = req.body.id; // uttam
        const userId = req.params.id; // mohit, yash
        const loggedInUser = await User.findById(loggedInUserId); // uttam
        const user = await User.findById(userId); // mohit, yash
